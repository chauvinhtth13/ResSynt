// ContactEndCaseCRF Form Audit Logging
// Tích hợp với base.js để ghi lại log cho form kết thúc nghiên cứu (Contact)

window.ContactEndCaseCRFAudit = {
  isProcessing: false,

  init: function() {
    console.log("Initializing ContactEndCaseCRF Form Audit Logging");

    const form = document.getElementById('contactendcaseForm');
    if (!form) {
      console.warn("ContactEndCaseCRF form not found");
      return;
    }
    ...
    console.log("ContactEndCaseCRF Form Audit Logging initialized successfully");
  },
  ...
  resetProcessingState: function() {
    ...
    const saveButton = document.getElementById('btnSaveContactEndcase');
    if (saveButton) {
  ...
  setupInitialValues: function() {
    ...
    const form = document.getElementById('contactendcaseForm');
    if (!form) {
      console.error("ContactEndCaseCRF form not found for initial values setup");
      return;
    }

    const fields = document.querySelectorAll('#contactendcaseForm input:not([type="hidden"]), #contactendcaseForm select, #contactendcaseForm textarea');
  ...
  setupFormSubmission: function() {
    ...
    $(document).off('click', '#btnSaveContactEndcase');
  ...
  collectFormData: function(useInitialValues = false) {
    ...
    const form = document.getElementById('contactendcaseForm');
    if (!form) {
      console.error("Form contactendcaseForm not found");
      return data;
    }
    ...
    fields.forEach(field => {
      ...
      if (name && name !== 'csrfmiddlewaretoken' && 
          ...
          name !== 'reasonsJson' && name === 'change_reason') return;
  ...
  getFieldLabels: function() {
    return {
      main: {
        ...
        'V2COMPLETED': 'V2 (Ngày 28±3)',
        'V3COMPLETED': 'V3 (Ngày 90±3)',
  ...
  showChangeModal: function(changedFields, callback) {
    console.log("ContactEndCaseCRFAudit.showChangeModal called with:", changedFields);
  ...
  saveAuditData: function(changedFields, reasonsData, callback) {
    ...
    console.log("Audit data saved for form contactendcaseForm:", { oldData, newData, reasonsJsonWithLabel, changeReason });
  ...
  submitForm: function(isCreate = false) {
    const form = document.getElementById('contactendcaseForm');
    const $btn = $('#btnSaveContactEndcase');
    const originalHtml = $btn.html();
    const usubjid = form.action.match(/\/43en\/contact\/([^/]+)\/endcasecrf\/(update|create)\//)?.[1] || window.location.pathname.split('/')[3];
    ...
    const url = isCreate ? `/43en/contact/${usubjid}/endcasecrf/create/` : `/43en/contact/${usubjid}/endcasecrf/update/`;
    ...
    $.ajax({
      ...
      success: (response) => {
        ...
        if (response.success) {
          ...
          setTimeout(() => {
            window.location.href = response.redirect_url || `/43en/contact/${usubjid}/`;
          }, 500);
        } else {
...
$(document).ready(function() {
  ...
  if (!isViewOnly) {
    console.log("Initializing ContactEndCaseCRFAudit");
    window.ContactEndCaseCRFAudit.init();
  } else {
