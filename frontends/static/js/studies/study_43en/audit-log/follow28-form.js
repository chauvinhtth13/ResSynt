// Follow Up Form Audit Logging
// Tích hợp với base.js để ghi lại log cho form theo dõi 28 ngày

window.FollowUpFormAudit = {
  isProcessing: false,

  init: function() {
    console.log("Initializing Follow Up Form Audit Logging");

    const form = document.getElementById('followUpForm');
    if (!form) {
      console.warn("Follow Up form not found");
      return;
    }
    ...
    console.log("Follow Up Form Audit Logging initialized successfully");
  },
  ...
  setupInitialValues: function() {
    ...
    const form = document.getElementById('followUpForm');
    if (!form) {
      console.error("Follow Up form not found for initial values setup");
      return;
    }
  ...
  setupFormsetInitialValues: function() {
    // Đảm bảo selector đúng cho formsets
    const rehospitalizationForms = document.querySelectorAll('#rehospitalization-formset .rehospitalization-form');
    console.log(`Found ${rehospitalizationForms.length} rehospitalization forms`);
    ...
    rehospitalizationForms.forEach((form, index) => {
      ...
      inputs.forEach(input => {
        ...
        if (name && !name.includes('-DELETE') && !name.includes('-id') && !name.includes('-USUBJID')) {
          ...
          console.log(`Set initial value for rehospitalization ${name}: ${initialValue}`);
        }
      });
    });

    const antibioticForms = document.querySelectorAll('#antibiotic-formset .antibiotic-form');
    console.log(`Found ${antibioticForms.length} antibiotic forms`);
    ...
    antibioticForms.forEach((form, index) => {
      ...
      inputs.forEach(input => {
        ...
        if (name && !name.includes('-DELETE') && !name.includes('-id') && !name.includes('-USUBJID')) {
          ...
          input.setAttribute('data-initial-value', initialValue);
          console.log(`Set initial value for antibiotic ${name}: ${initialValue}`);
        }
      });
    });
  },
  ...
  validateForm: function() {
    ...
    if ($('input[name="REHOSP"]:checked').val() === 'Yes') {
      ...
      $('#rehospitalization-formset .rehospitalization-form').each(function() {
        if (!$(this).find('input[id$="-DELETE"]').val() && (
  ...
  collectFormData: function(useInitialValues = false) {
      ...
      // Thu thập dữ liệu từ formset rehospitalization
      ...
      rehospitalizationRows.forEach((row, rowIndex) => {
          ...
          // Kiểm tra nếu form đã bị đánh dấu xóa
          ...
          inputs.forEach(input => {
              const name = input.name;
              if (name && !name.includes('-DELETE') && !name.includes('-id') && !name.includes('-follow_up')) {
                  // Lấy tên trường từ cấu trúc rehospitalization_set-0-FIELDNAME
                  const fieldMatch = name.match(/rehospitalization_set-\d+-(\w+)/);
      ...
      // Thu thập dữ liệu từ formset antibiotic
      ...
      antibioticRows.forEach((row, rowIndex) => {
          ...
          inputs.forEach(input => {
              const name = input.name;
              if (name && !name.includes('-DELETE') && !name.includes('-id') && !name.includes('-follow_up')) {
                  // Lấy tên trường từ cấu trúc antibiotic_set-0-FIELDNAME
                  const fieldMatch = name.match(/antibiotic_set-\d+-(\w+)/);
                  if (fieldMatch && fieldMatch[1]) {
                      const fieldName = fieldMatch[1];
                      let value = '';
                      if (useInitialValues) {
  ...
  compareData: function(initialData, currentData) {
    ...
    // So sánh formset rehospitalization
    ...
    for (let i = 0; i < rehospitalizationMaxLength; i++) {
      ...
      Object.keys(fieldLabels.rehospitalization || {}).forEach(field => {
        const fieldKey = `rehospitalization_${i}_${field}`;
    ...
    for (let i = 0; i < antibioticMaxLength; i++) {
      ...
      Object.keys(fieldLabels.antibiotic || {}).forEach(field => {
        const fieldKey = `antibiotic_${i}_${field}`;
        const oldValue = AuditLogBase.normalizeValue(oldRow[field] || '');
        const newValue = AuditLogBase.normalizeValue(newRow[field] || '');
        if (oldValue !== newValue) {
          changedFields[fieldKey] = {
  ...
  getFieldLabels: function() {
    return {
      main: {
        'ASSESSED': 'Bệnh nhân được đánh giá tình trạng tại ngày 28?',
        ...
        'FUNCASSESS': 'Đánh giá tình trạng chức năng tại ngày 28?',
  ...
  showChangeModal: function(changedFields, callback) {
    console.log("FollowUpFormAudit.showChangeModal called with:", changedFields);
  ...
  saveAuditData: function(changedFields, reasonsData, callback) {
      ...
      // Tạo dữ liệu PHẲNG từ dữ liệu ban đầu của form để phù hợp với cách xử lý của backend
      const initialData = this.collectFormData(true);
      const currentData = this.collectFormData(false);
      
      // Làm phẳng dữ liệu cũ
      const oldData = {};
      // Dữ liệu chính
      ...
      initialData.rehospitalization.forEach((item, idx) => {
          if (item) {
              for (const field in item) {
                  oldData[`rehospitalization_${idx}_${field}`] = item[field];
              }
          }
      });
      // Dữ liệu kháng sinh
      initialData.antibiotic.forEach((item, idx) => {
          if (item) {
              for (const field in item) {
                  oldData[`antibiotic_${idx}_${field}`] = item[field];
              }
          }
      });

      // Làm phẳng dữ liệu mới
      const newData = {};
      // Dữ liệu chính
      for (const key in currentData.main) {
      ...
      currentData.rehospitalization.forEach((item, idx) => {
          if (item) {
              for (const field in item) {
                  newData[`rehospitalization_${idx}_${field}`] = item[field];
              }
          }
      });
      // Dữ liệu kháng sinh
      currentData.antibiotic.forEach((item, idx) => {
          if (item) {
              for (const field in item) {
                  newData[`antibiotic_${idx}_${field}`] = item[field];
              }
          }
      });

      // Xử lý reasons như trước đây
      ...
      Object.keys(changedFields).forEach(key => {
          ...
          } else if (key.includes('rehospitalization_')) {
              // Xử lý cho các trường trong rehospitalization formset
              const [prefix, index, field] = key.split('_');
              const episode = parseInt(index) + 1;
              
              // Thử các định dạng khác nhau cho rehospitalization
              const possibleKeys = [
                  `rehospitalization-${episode}-${field}`,
                  `rehospitalization_set-${index}-${field}`,
                  `rehospitalization_${index}_${field}`,
                  `${field}`
              ];
          ...
          } else if (key.includes('antibiotic_')) {
              // Xử lý cho các trường trong antibiotic formset
              const [prefix, index, field] = key.split('_');
              const episode = parseInt(index) + 1;
              
              // Thử các định dạng khác nhau cho antibiotic
              const possibleKeys = [
                  `antibiotic-${episode}-${field}`,
                  `antibiotic_set-${index}-${field}`,
                  `antibiotic_${index}_${field}`,
                  `${field}`
              ];
      ...
      // Tạo chuỗi lý do thay đổi tổng hợp
  ...
  submitForm: function() {
    ...
    $.ajax({
      url: `/43en/${usubjid}/followup/update/`,
      method: 'POST',
      data: formData,
      processData: false,
      contentType: false,
      success: (response) => {
        ...
        if (response.success) {
          this.showMessage('Đã cập nhật thông tin theo dõi 28 ngày thành công!', 'success');
          setTimeout(() => {
  ...
  setupSpecialHandlers: function() {
    ...
    const formsetTables = document.querySelectorAll('#followUpForm table[id$="-formset"]');
...
$(document).ready(function() {
  ...
  if (!isViewOnly) {
    console.log("Initializing FollowUpFormAudit");
    window.FollowUpFormAudit.init();
  } else {
