...
(function() {
  ...
  function updateAddressPreview() {
    ...
    if (selectedSystem === 'old' || selectedSystem === 'both') {
      const houseNumber = $('#full-address-input').val()?.trim();
      // Always prefer #street-autocomplete for smart address
      const street = $('#street-autocomplete').val()?.trim() || $('input[name="STREET"]').val()?.trim();
      const ward = $('#ward-hidden-input').val()?.trim() || $('input[name="WARD"]').val()?.trim();
      const district = $('#district-hidden-input').val()?.trim() || $('input[name="DISTRICT"]').val()?.trim();
      const city = $('#city-input').val()?.trim() || $('input[name="PROVINCECITY"]').val()?.trim();
      ...
      if (houseNumber || street || ward || district || city) {
        ...
        if (houseNumber && street) {
          houseAndStreet = houseNumber + ' ' + street;
        } else if (houseNumber) {
          houseAndStreet = houseNumber;
        } else if (street) {
          houseAndStreet = street;
        }
        if (houseAndStreet) addressParts.push(houseAndStreet);
