...
(function () {
    ...
    const ChartRenderer = {
        ...
        /**
         * Calculate smart Y-axis configuration for even divisions
         * Special case: 680-750 range uses interval 50
         * @param {number} target - Site target value
         * @param {Array} targetData - Target data array
         * @param {Array} actualData - Actual data array
         * @returns {Object} - { max, interval }
         */
        calculateYAxisConfig(target, targetData, actualData) {
            // Find the maximum value from target, actual data, and site target
            const maxFromData = Math.max(
                target || 0,
                ...targetData.filter(v => v !== null && v !== undefined),
                ...actualData.filter(v => v !== null && v !== undefined)
            );
            
            // Special case for All Sites (680-750 range): use interval 50
            if (maxFromData >= 650 && maxFromData <= 800) {
                const max = Math.ceil(maxFromData / 50) * 50;
                return { max, interval: 50 };
            }
            
            // For other values, calculate smart intervals
            // Goal: 4-7 grid lines for clean look
            let interval, max;
            
            if (maxFromData <= 30) {
                interval = 5;
            } else if (maxFromData <= 60) {
                interval = 10;
            } else if (maxFromData <= 120) {
                interval = 20;
            } else if (maxFromData <= 200) {
                interval = 25;
            } else if (maxFromData <= 400) {
                interval = 50;
            } else if (maxFromData <= 650) {
                interval = 100;
            } else {
                interval = 100;
            }
            ...
            max = Math.ceil(maxFromData * 1.05 / interval) * interval;
            
            // Ensure max is at least the target
            if (max < target) {
                max = Math.ceil(target / interval) * interval;
            }
        ...
        buildChartOptions(data, container) {
            ...
            // Calculate smart Y-axis configuration
            const yAxisConfig = this.calculateYAxisConfig(data.site_target, target, actual);
            ...
            return {
                ...
                // Y Axis - Dynamic range with smart intervals
                yAxis: {
                    type: 'value',
