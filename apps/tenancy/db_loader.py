...
def load_study_dbs(force_refresh=False):
    ...
    lock_key = 'study_dbs_lock'
    study_dbs = cache.get(cache_key)
    acquired_lock = False

    if study_dbs is None or force_refresh:
        # Acquire lock to prevent thundering herd on cache miss
        ...
        if not acquired_lock:
            logger.warning("Could not acquire lock for loading study DBs; using stale if available.")
            study_dbs = cache.get(cache_key) or {}

        if acquired_lock or force_refresh:
            study_dbs = {}
            try:
