# backends/api/studies/study_43en/views/patient/discharge/helpers.py
"""
Discharge Helper Functions.

Provides transaction handlers and ICD code processing.
"""
import logging
...
from backends.studies.study_43en.models.patient import (
    SCR_CASE, ENR_CASE, DISCH_CASE, DischargeICD,
)
...
def get_discharge_with_related(request, usubjid):
    ...
    try:
        ...
        ).prefetch_related('icd_codes').get(USUBJID=enrollment)
        
        logger.debug(f"Loaded discharge with {discharge_case.icd_code_count} ICD codes")
        return screening, enrollment, discharge_case
