...
# backends/studies/study_43en/views/patient/FU28/helpers.py
"""
...
def get_followup_with_related(request, usubjid):
    ...
    try:
        followup_case = FU_CASE_28.objects.select_related(
            'USUBJID',
            'USUBJID__USUBJID'
        ).prefetch_related(
            'rehospitalizations',
        ...
        return screening_case, enrollment_case, followup_case
...
def save_followup_and_related(
    ...
    try:
        with transaction.atomic():
            ...
            logger.info(
                f"{'Created' if is_create else 'Updated'} follow-up Day 28: "
...
def _save_rehospitalization_formset(formset, followup_case, user):
    ...
    formset.save_m2m()
    logger.info(f"Saved {len(instances)} rehospitalizations")
...
def _save_antibiotic_formset(formset, followup_case, user):
    ...
    formset.save_m2m()
    logger.info(f"Saved {len(instances)} antibiotics")
