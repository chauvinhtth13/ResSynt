...
class FU_CASE_90(AuditFieldsMixin):
    """
    Follow-up Case at Day 90
    ...
    # ==========================================
    EvaluatedAtDay90 = models.CharField(
        max_length=3,
        choices=AssessedChoices.choices,
        null=True,
        blank=True,
        db_index=True,
        verbose_name=_("1. The patient's condition was assessed on Day 90?")
    )
    ...
    Outcome90Days = models.CharField(
        max_length=20,
    ...
    Func_Status = models.CharField(
        ...
        null=True,
        blank=True,
        verbose_name=_('5. Assessment of Functional Status at Day 90?')
    )
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=['EvaluateDate'], name='idx_fu90_assess'),
            models.Index(fields=['Outcome90Days'], name='idx_fu90_status'),
            models.Index(fields=['EvaluatedAtDay90', 'EvaluateDate'], name='idx_fu90_asses_date'),
            models.Index(fields=['last_modified_by_id', '-last_modified_at'], name='idx_fu90_modified'),
        ]
        constraints = [
            # If assessed YES, must have assessment date
            models.CheckConstraint(
                condition=(
                    ~models.Q(EvaluatedAtDay90='Yes') |
                    models.Q(EvaluateDate__isnull=False)
                ),
                name='fu90_assess_date_required'
            ),
            ...
            models.CheckConstraint(
                ...
                name='fu90_death_info_required'
            ),
        ]
    
    def __str__(self):
        status = self.Outcome90Days or 'Unknown'
        return f"FU Day 90: {self.USUBJID_id} - {status}"
    ...
    def is_alive(self):
        ...
        return self.Outcome90Days == self.PatientStatusChoices.ALIVE
    
    @cached_property
    def is_deceased(self):
        """Check if patient is deceased"""
        return self.Outcome90Days == self.PatientStatusChoices.DECEASED
    ...
    def clean(self):
        ...
        if self.EvaluatedAtDay90 == self.AssessedChoices.YES:
        ...
        if self.Dead == self.AssessedChoices.YES and self.Outcome90Days != self.PatientStatusChoices.DECEASED:
            errors['Outcome90Days'] = _('Patient status must be "Deceased" when DECEASED is "Yes"')
    ...
    def save(self, *args, **kwargs):
        ...
        if self.Dead == self.AssessedChoices.YES:
            self.Outcome90Days = self.PatientStatusChoices.DECEASED
    ...
    def get_assessed_cases(cls):
        ...
        return cls.objects.filter(
            EvaluatedAtDay90=cls.AssessedChoices.YES
    ...
    def get_alive_cases(cls):
        """Get all alive cases"""
        return cls.objects.filter(
            Outcome90Days=cls.PatientStatusChoices.ALIVE
        ).select_related('USUBJID')
    ...
    def get_lost_to_followup(cls):
        ...
        return cls.objects.filter(
            Outcome90Days=cls.PatientStatusChoices.LOST_TO_FOLLOWUP
        ).select_related('USUBJID')