...
class ContactEndCaseCRF(AuditFieldsMixin):
    """
    Contact End of Study Case Report Form
    Final study completion status and visit tracking for contacts
    
    Differences from Patient EndCaseCRF:
    ...
    ENDDATE = models.DateField(
        ...
        help_text=_('Date when contact ended study participation')
    )
    ...
    # ==========================================
    # VISIT COMPLETION STATUS (3 visits for contacts)
    # ==========================================
    ...
    V2COMPLETED = models.BooleanField(
        ...
        help_text=_('Contact V2 is at Day 28, not Day 10 like patients')
    )
    ...
    WITHDRAWDATE = models.DateField(
        ...
        help_text=_('Date when contact withdrew from study')
    )
    ...
    LOSTTOFOLLOWUPDATE = models.DateField(
        ...
        help_text=_('Date when contact was determined lost to follow-up')
    )
    ...
    class Meta:
        ...
        ordering = ['-ENDDATE']
        indexes = [
            models.Index(fields=['ENDDATE'], name='idx_cec_enddate'),
            models.Index(fields=['WITHDRAWREASON'], name='idx_cec_withdraw'),
            models.Index(fields=['INCOMPLETE'], name='idx_cec_incomplete'),
            models.Index(fields=['LOSTTOFOLLOWUP'], name='idx_cec_ltfu'),
            models.Index(fields=['STUDYCOMPLETED', 'ENDDATE'], name='idx_cec_comp_date'),
            models.Index(fields=['last_modified_by_id', '-last_modified_at'], name='idx_cec_modified'),
        ]
        constraints = [
            ...
            models.CheckConstraint(
                ...
                name='cec_withdraw_date_required'
            ),
            ...
            models.CheckConstraint(
                ...
                name='cec_incomplete_reason_required'
            ),
            ...
            models.CheckConstraint(
                ...
                name='cec_ltfu_date_required'
            ),
        ]
    
    def __str__(self):
        return f"Contact End Case - {self.USUBJID.USUBJID if hasattr(self.USUBJID, 'USUBJID') else self.USUBJID_id}"
    ...
    def total_visits_completed(self):
        """Count total number of completed visits (3 for contacts)"""
        return sum([
            self.VICOMPLETED,
            self.V2COMPLETED,
            self.V3COMPLETED,
    ...
    def completion_rate(self):
        """Calculate visit completion rate (0-100% based on 3 visits)"""
        return (self.total_visits_completed / 3) * 100
    
    @cached_property
    def is_withdrawn(self):
        """Check if contact withdrew from study"""
    ...
    def is_lost_to_followup(self):
        """Check if contact is lost to follow-up"""
    ...
    def visit_completion_summary(self):
        """Get summary of visit completions (3 visits)"""
        return {
            'V1': self.VICOMPLETED,
            'V2': self.V2COMPLETED,
            'V3': self.V3COMPLETED,
    ...
    def all_visits_completed(self):
        """Check if all visits were completed (3 visits)"""
        return all([
            self.VICOMPLETED,
            self.V2COMPLETED,
            self.V3COMPLETED,
    ...
    def clean(self):
        ...
        if self.is_withdrawn:
            if not self.WITHDRAWDATE:
                errors['WITHDRAWDATE'] = _('Withdrawal date is required when contact withdrew')
        ...
        # Validate study completion status consistency (3 visits)
        if self.STUDYCOMPLETED:
            if not self.all_visits_completed:
                errors['STUDYCOMPLETED'] = _(
                    'Cannot mark study as completed when not all 3 visits are completed'
                )
            if self.has_early_termination:
                errors['STUDYCOMPLETED'] = _(
                    'Cannot mark study as completed when contact withdrew, was lost to follow-up, or study is incomplete'
                )
    ...
    def save(self, *args, **kwargs):
        ...
        # Auto-set study completion flag (3 visits)
        self.STUDYCOMPLETED = (
    ...
    def get_completion_statistics(cls):
        """
        Get study completion statistics for contacts
        
        OPTIMIZED: Single aggregate query instead of 8 separate count queries
    ...
    def get_cases_by_completion_rate(cls, min_rate=0, max_rate=100):
        ...
        Returns:
            List of ContactEndCaseCRF instances
        """
        cases = []
...
# UTILITY FUNCTIONS
# ==========================================
def generate_contact_completion_report(site_id=None, start_date=None, end_date=None):
    """
    Generate comprehensive completion report for contacts
    ...
    report = {
        ...
        'overall': ContactEndCaseCRF.get_completion_statistics(),
        'duration_analysis': ContactEndCaseCRF.get_duration_analysis(),
        'withdrawal_reasons': {},
        'incomplete_reasons': {},
    }
    
    # Withdrawal reasons breakdown
    for reason in ContactEndCaseCRF.WithdrawReasonChoices.values:
